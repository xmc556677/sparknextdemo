...
import cc.xmccc.sparkdemo.schema.NewSessionFeatureTable
...
object ExtractSessionFeature2 {
  ...
  def stream_statitic_pattern(datas: List[(BigInt, Int, Option[(BigInt, BigInt)], Option[Int])]) = {
    ...
    val avg_pkt_len = Try{pkt_len.sum / pkt_len.length}.getOrElse(0)
    val min_pkt_len = pkt_len.min
    val max_pkt_len = pkt_len.max
    val var_pkt_len = Try{pkt_len.map(x => (x - avg_pkt_len) ^ 2).sum / pkt_len.length.toDouble}.getOrElse(0.toDouble)
  ...
  def SessionFeatureExtract(pkts_rdd: RDD[(Array[Byte], Option[Array[Byte]], Array[Byte], Array[Byte])]): RDD[NewSessionFeatureTable] = {
    ...
    val save_rdd = sid_group_rdd.map{
      case(_, sessn_datas) =>
        ...
        val min_ts_IAT = Try{ts_IAT.min}.getOrElse(BigInt(0))
        val max_ts_IAT = Try{ts_IAT.max}.getOrElse(BigInt(0))
        val var_ts_IAT = Try{(BigDecimal(ts_IAT.map(x => math.pow((x - avg_ts_IAT).toDouble, 2)).sum) / ts_IAT.length).toDouble}.getOrElse(0.toDouble)
  ...
  def main(args: Array[String]): Unit = {
    val sparkSession = SparkSession.builder()
      .appName("ExtractSessionFeature2")
      .getOrCreate()
    ...
    if (! admin.tableExists(TableName.valueOf(output_table)))
      createPresplitTable(output_table, "sessn")

    val pkts_rdd = sparkSession.sparkContext.hbaseTable[(Array[Byte], Option[Array[Byte]], Array[Byte], Array[Byte])](input_table)
      .select("sid", "t", "r" )
      .inColumnFamily("p")

    val sessn_feature_rdd = SessionFeatureExtract(pkts_rdd)

    sessn_feature_rdd.toHBaseTable(output_table)
      .toColumns(
