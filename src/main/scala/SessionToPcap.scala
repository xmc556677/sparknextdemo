...
object SessionToPcap {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val packet_table = sparkSession.sparkContext.hbaseTable[(Array[Byte], Array[Byte], Array[Byte], Option[Array[Byte]], Option[Array[Byte]])](table)
      .select("r", "t", "sid", "m")
      .inColumnFamily("p")
      .filter{
        _ match {
          case(_, _, _, Some(_), _) => true
          case _ => false
        }
      }.map(item => (item._1, item._2, item._3, item._4.get, item._5))
